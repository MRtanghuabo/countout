...
<script>
    ...
    function pageStart(pageNum,pageSize,primaryCustomerName,customerQueryName,dateBegin,dateEnd){
        ...
        $.ajax({
                ...
                contentType: "application/json;charset=utf-8",
                data:JSON.stringify({"topCustomerId":primaryCustomerName,"applyCustomerId":customerQueryName,"dateBegin":dateBegin,"profitCustomerLevel":1,"dateEnd":dateEnd,"pageNum":pageNum,"pageSize":pageSize}),
                success:function(data){
                    ...
                    $(".pages-size").html("共<em><i>"+data.data.pageNum+"</i>/<i class='pageAllnum'>"+data.data.pages+"</i></em>页")
                     $.each(data.data.list,function(i,item){         	 
                      	...
                      	<td>"+isNotNullAndUndifine(item.topCustomerRateStr)+"</td>\
                      	<td>"+isNotNullAndUndifine(item.profit)+"</td>\
                      	<td><a target='_blank' href="+item.rechargeImgUrl+" >查看</a></td></tr>";
